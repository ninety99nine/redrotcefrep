...
Route::middleware(['auth:sanctum'])
    ...
    ->group(function () {
        ...
        Route::get('/alias/{alias}', 'showStoreByAlias')
                ->middleware(['record.store.visit'])
                ->withoutMiddleware(['auth:sanctum', 'store.permission'])->name('show.store.by.alias');

        // Explicit route model binding applied: AppServiceProvider.php
        Route::middleware(['store.permission'])->prefix('{store}')->group(function () {
            //  Allow Guest shopping on showStore
            Route::get('/', 'showStore')
                ->middleware(['record.store.visit'])
                ->withoutMiddleware(['auth:sanctum', 'store.permission'])->name('show.store');
