...
class CreateDeliveryMethodRequest extends FormRequest
{
    ...
    {
        return $this->user()->can('create', DeliveryMethod::class);
    }
    ...
    public function rules(): array
    {
        return [
            ...
            'percentage_fee_rate' => ['nullable', 'numeric', 'min:0', 'max:99.99'],
            ...
            'fallback_percentage_fee_rate' => ['nullable', 'numeric', 'min:0', 'max:99.99'],
            ...
            'operational_hours.*.day' => ['required_with:operational_hours', 'string', Rule::enum(DaysOfTheWeek::class)],
            'operational_hours.*.open' => ['required_with:operational_hours', 'string', 'regex:/^([01]\d|2[0-3]):[0-5]\d$/'],
            'operational_hours.*.close' => ['required_with:operational_hours', 'string', 'regex:/^([01]\d|2[0-3]):[0-5]\d$/'],
    ...
    {
        return [
            'name.required' => 'The delivery method name is required.',
            ...
            'operational_hours.*.close.regex' => 'The closing time must be in HH:MM format (e.g., 17:00).',
